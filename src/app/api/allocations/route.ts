import { getTeams, getProjects, getTeamEstimates, getContractors, getPriorityOverrides } from '@/lib/store';
...
export async function GET(req: Request) {
  ...
  if (scenarioId) {
    for (const po of getPriorityOverrides(scenarioId)) {
      priorityOverrides[po.projectId] = po.priority;
    }
    contractors = getContractors(scenarioId).map(c => ({
